...
- **紫色预算卡 → 直接弹出总预算/目标设置。**
- **每日消费趋势图本体 → 交互查看数据，不再直接跳页。**
- 趋势图右上角 **“查看明细” → 消费明细页。**
- 趋势图支持短按查看单点、长按进入悬停、左右拖动切换日期与金额。
...
### 2.3.1 每日消费趋势图（v1.6 起）

趋势图必须保持“图表优先、明细入口显式”的交互：

- 图表加载时有约 700ms 左到右绘制动画。
- 页面切换使用轻量淡入 + 上移动画。
- 图表必须显示 Y 轴金额刻度、X 轴日期刻度。
- X 轴默认均匀显示最多 5 个清晰日期标签（`MM/dd`）。
- Y 轴使用自动 nice scale，显示 `¥0 / ¥500 / ¥1k` 等易读金额。
- 短按图表：显示最近日期的数据气泡，不跳页。
- 长按约 260ms：震动反馈并进入数据悬停。
- 长按后左右拖动：跟随手指切换日期、金额、十字指示线。
- 松手后 tooltip 保留约 1.8 秒再自动消失。
- “查看明细 ›”为唯一显式进入消费明细页的入口。
...
用户验收时第一步必须确认手机“关于我们”显示 `1.6.0 (10)`。
...
- [ ] 首页趋势线有绘制动画。
- [ ] 趋势图短按不会跳转。
- [ ] 长按趋势图出现日期 + 金额 tooltip。
- [ ] 长按后左右拖动可连续切换数据点。
- [ ] X 轴至少显示起点、中间点、终点日期且无遮挡。
- [ ] Y 轴金额刻度与网格线对应。
- [ ] “查看明细 ›”按钮可以进入消费明细页。
